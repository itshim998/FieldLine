...
backend/
  src/
    ...
    database/         # SQLite connection lifecycle, WAL pragmas, migrator, and migrations/
      migrations/     # Ordered migrations (0001_baseline_system_metadata.ts, 0002_core_domain_schema.ts)
...
3. **Pass 2 — SQLite & Persistence Foundation**:
   - Migration engine (`schema_migrations` tracking).
   ...
   - Foreign key enforcement and cascading rules.
   ...
   - Comprehensive Vitest persistence test suite (13 test suites, 61 passing tests).
