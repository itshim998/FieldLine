...
export async function seedGoldenDemo(): Promise<SeedGoldenDemoResult> {
  ...
  let project = await projectRepository.getByCode(goldenProjectManifest.code);
  ...
  try {
    ...
    await authService.provisionDefaultAccounts(projectId, {
    ...
    const report14 = await progressUpdateRepository.create({
      projectId,
    ...
    await evidenceRepository.attachToProgressUpdate(ev14.id, report14.id, projectId);
    ...
    for (const obs of observations14) {
      ...
      const match = await activityMatchRepository.create({
        projectId,
      ...
      await progressService.normalizeAndRecordProgress({
        projectId,
    ...
    const report18 = await progressUpdateRepository.create({
      projectId,
    ...
    await evidenceRepository.attachToProgressUpdate(ev18.id, report18.id, projectId);
    ...
    const report21 = await progressUpdateRepository.create({
      projectId,
    ...
    await evidenceRepository.attachToProgressUpdate(ev21.id, report21.id, projectId);
    ...
    const report24 = await progressUpdateRepository.create({
      projectId,
    ...
    await evidenceRepository.attachToProgressUpdate(ev24.id, report24.id, projectId);
    ...
    await activityMatchRepository.create({
      projectId,
      progressUpdateId: report24.id,
      evidenceId: ev24.id,
    ...
    const report27 = await progressUpdateRepository.create({
      projectId,
    ...
    await evidenceRepository.attachToProgressUpdate(ev27.id, report27.id, projectId);
    ...
    const report28 = await progressUpdateRepository.create({
      projectId,
    ...
    await evidenceRepository.attachToProgressUpdate(ev28.id, report28.id, projectId);

    // 5. Emit Audit Project Events
    await projectEventRepository.create({
      projectId,
    ...
    const existingBlockers = await operationalBlockerRepository.listByProjectId(projectId);
    for (const eb of existingBlockers) {
      await operationalBlockerRepository.delete(eb.id, projectId);
    }
    ...
    for (const gb of goldenOperationalBlockers) {
      ...
      await operationalBlockerRepository.create({
        projectId,
    ...
    if (isNewlyCreated) {
      try {
        logger.warn(`Cleaning up partially seeded project [${projectId}] after error...`);
        await projectRepository.delete(projectId);
      } catch (cleanupErr) {
        logger.error('Failed to clean up partial project:', cleanupErr);
      }
    }
    throw err;
  }
